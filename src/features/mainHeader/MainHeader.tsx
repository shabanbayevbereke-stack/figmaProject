...
export function MainHeader() {
  ...
  return (
    <div
      className={`flex h-screen w-full overflow-hidden ${isDark ? "bg-[#020617]" : "bg-slate-50"}`}
    >
      <div
        className={`transition-all duration-300 border-r ${themeClass} ${isOpen ? "w-64" : "w-0 overflow-hidden"}`}
      >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className={headerClass}>
          <div className="flex items-center gap-4">
            ...
            >
              {isOpen ? <SquareChevronLeft size={20} /> : <FiMenu size={20} />}
            </button>
          ...
          <div className="flex items-center gap-5">
            <div className="text-right hidden sm:block">
              <p className="text-sm font-semibold leading-none">
                {data?.firstName || "Пользователь"}
              </p>
              {data?.roleName && (
                <p className="text-xs text-slate-500 mt-1">{data?.roleName}</p>
              )}
            </div>
